...
# Borrowed from https://github.com/aanjaa/gflownet/blob/trunk/src/gflownet/utils/metrics_final_eval.py
def compute_num_of_modes(candidates, reward_thresh=8, tanimoto_thresh=0.7):
    # Candidate is pair (real_reward, smiles)
    ...
    modes = [candidates[0]]
    mode_fps = [Chem.RDKFingerprint(candidates[0][1].mol)]
    for i in range(1, len(candidates)):
        fp = Chem.RDKFingerprint(candidates[i][1].mol)
